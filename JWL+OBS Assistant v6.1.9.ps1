...
$script:_cutHotKeyId = 0              # Registered hotkey ID (0 = not registered)
$script:_cutHotKeyFKey = 'F8'         # Current F-key (F1, F2, F8, F9, F12)
...
if (-not ([System.Management.Automation.PSTypeName]'HotKeyAPI').Type) {
    Add-Type -MemberDefinition $HotKeyDeclaration -Name "HotKeyAPI" -Namespace "User32"
}

# IMessageFilter class that intercepts WM_HOTKEY messages from the application message loop
# Class name includes version to avoid stale cached type from previous session runs
$HotKeyFilterDeclaration = @'
using System;
using System.Windows.Forms;

public class JwlHotKeyFilter : IMessageFilter {
    private const int WM_HOTKEY = 0x0312;
    // Static flag: set by the message filter, polled by a WinForms timer on the UI thread
    public static volatile int LastHotKeyId = 0;
    
    public JwlHotKeyFilter() { }
...
if (-not ([System.Management.Automation.PSTypeName]'JwlHotKeyFilter').Type) {
    try {
        $primitivesAsm = [System.Windows.Forms.Message].Assembly.Location
        Add-Type -TypeDefinition $HotKeyFilterDeclaration -ReferencedAssemblies @(
...
$script:_hotKeyFilter = $null   # Holds the active IMessageFilter instance
$script:_hotKeyPollTimer = $null # Timer that polls the static flag set by the message filter

# Virtual Key codes for F-keys
$script:FKeyVirtualKeyCodes = @{
    'F1'  = 0x70
    'F2'  = 0x71
    'F8'  = 0x77
    'F9'  = 0x78
...
function Convert-TagToVersion([string]$tag) {
    try {
        ...
        # Handle vMAJOR.MINOR.PATCH[LETTER][NUMBER] e.g. v6.1.8f, v6.1.8b7, v6.1.9
        # Encode letter suffix as ordinal * 100 + trailing number into 4th component
        ...
        # Fallback: extract all digit sequences
        $digits = [regex]::Matches($tag, '\d+')
        if (-not $digits -or $digits.Count -eq 0) { return $null }
        $nums = @()
        foreach ($hit in $digits) { $nums += [int]$hit.Value }
...
$script:Cfg = [ordered]@{
    ...
    OBSControl = [ordered]@{ AutoStartAutoToggle = $false; AutoToggleLeadSeconds = 10; AutoVirtualCamera = $false; AutoVirtualCameraSeconds = 20; RecordingConfigured = $false; CutHotKeyEnabled = $false; CutHotKeyFKey = 'F8' }
...
function Load-Settings {
    ...
    try {
        ...
        if ($c.OBSControl) {
            foreach ($k in 'AutoStartAutoToggle', 'AutoToggleLeadSeconds', 'AutoVirtualCamera', 'AutoVirtualCameraSeconds', 'CutHotKeyEnabled', 'CutHotKeyFKey') {
                if ($c.OBSControl.PSObject.Properties.Name -contains $k) { $script:Cfg.OBSControl[$k] = $c.OBSControl.$k }
            }
        }
        
        # Ensure Cut hotkey FKey has a valid default
        if ([string]::IsNullOrWhiteSpace([string]$script:Cfg.OBSControl.CutHotKeyFKey)) {
            $script:Cfg.OBSControl.CutHotKeyFKey = 'F8'
        }
...
# ---- Cut Hotkey Functions ----
function Register-CutHotKey {
    if (-not $script:form) { return }
    if ($script:_cutHotKeyId -gt 0) { Unregister-CutHotKey }
    
    $enabled = [bool]$script:Cfg.OBSControl.CutHotKeyEnabled
    $fkey = [string]$script:Cfg.OBSControl.CutHotKeyFKey
    
    if (-not $enabled -or [string]::IsNullOrWhiteSpace($fkey)) { return }
    
    $vk = $script:FKeyVirtualKeyCodes[$fkey]
    if (-not $vk) { Log "Cut hotkey: invalid F-key '$fkey'"; return }
    
    try {
        $hwnd = $script:form.Handle
        $hotKeyId = 1001   # arbitrary ID for Cut hotkey
        
        # MOD_NONE = 0 (no modifiers: bare F-key)
        $result = [User32.HotKeyAPI]::RegisterHotKey($hwnd, $hotKeyId, 0, $vk)
        
        if ($result) {
            $script:_cutHotKeyId = $hotKeyId
            $script:_cutHotKeyFKey = $fkey
            Log "Cut hotkey registered: $fkey (VK=$vk)"
            
            # Install the message filter to intercept WM_HOTKEY on the UI thread
            if ($null -eq $script:_hotKeyFilter) {
                try {
                    $script:_hotKeyFilter = New-Object JwlHotKeyFilter
                    [System.Windows.Forms.Application]::AddMessageFilter($script:_hotKeyFilter)
                    
                    # Polling timer: reads the static flag every 50ms on the UI thread
                    if ($null -eq $script:_hotKeyPollTimer) {
            ...
            Log "Cut hotkey registration failed for $fkey"
        }
    }
    catch {
        Log "Cut hotkey registration error: $_"
    }
}

function Unregister-CutHotKey {
    if (-not $script:form -or $script:_cutHotKeyId -le 0) { return }
    
    try {
        # Remove the message filter and stop the poll timer
        if ($null -ne $script:_hotKeyPollTimer) {
            try { $script:_hotKeyPollTimer.Stop(); $script:_hotKeyPollTimer.Dispose() } catch {}
            $script:_hotKeyPollTimer = $null
        }
        if ($null -ne $script:_hotKeyFilter) {
            try { [System.Windows.Forms.Application]::RemoveMessageFilter($script:_hotKeyFilter) } catch {}
            $script:_hotKeyFilter = $null
        }
        
        $hwnd = $script:form.Handle
        [User32.HotKeyAPI]::UnregisterHotKey($hwnd, $script:_cutHotKeyId) | Out-Null
        Log "Cut hotkey unregistered: $script:_cutHotKeyFKey"
        $script:_cutHotKeyId = 0
    }
    catch {
        Log "Cut hotkey unregistration error: $_"
    }
}

function On-CutHotKeyPressed {
    # Called from a WinForms timer tick — already on the UI thread, so PerformClick directly
    try {
        if ($script:btnCut -and -not $script:btnCut.IsDisposed -and $script:btnCut.Enabled) {
            Log "Cut hotkey fired: performing Cut"
            $script:btnCut.PerformClick()
        }
    }
    catch {
        Log "Cut hotkey click error: $_"
    }
}
...
$btnZoomJoin.Add_Click({
        try {
            # Guard: prevent double-click while join is in progress
            if ($script:_zoomJoinInProgress) {
                Log "Zoom Join: already in progress, ignoring click"
                return
            }
            $script:_zoomJoinInProgress = $true
...
"@
    ...
    # ---- Cut Hotkey Enable + F-Key Selection ----
    $chkCutHotKey = New-Object System.Windows.Forms.CheckBox
    $chkCutHotKey.Text = "Enable 'Cut' hot key:"
    $chkCutHotKey.AutoSize = $true
    ...
    $lblCutFKey.Text = "F-key:"
    $lblCutFKey.AutoSize = $true
    $lblCutFKey.Location = Pt ([int]($chkCutHotKey.Left + $chkCutHotKey.PreferredSize.Width + 12)) 240
    $grpObsCtl.Controls.Add($lblCutFKey)
    
    $cmbCutFKey = New-Object System.Windows.Forms.ComboBox
    $cmbCutFKey.Items.AddRange(@('F1', 'F2', 'F8', 'F9', 'F12'))
    $cmbCutFKey.DropDownStyle = 'DropDownList'
    $cmbCutFKey.SelectedItem = [string]$script:Cfg.OBSControl.CutHotKeyFKey
    $cmbCutFKey.Size = Sz 60 24
    ...
    $lblCutFKeyTip.Text = "Choose from F1, F2, F8, F9, F12 (F3-F7 are reserved by Windows)   |   To unlock F-keys press FN+Esc for 3 seconds"
    $lblCutFKeyTip.AutoSize = $true
    $lblCutFKeyTip.Location = Pt 10 264
    $lblCutFKeyTip.ForeColor = [System.Drawing.Color]::DarkGreen
    $lblCutFKeyTip.Font = New-Object System.Drawing.Font($lblCutFKeyTip.Font.FontFamily, 8, [System.Drawing.FontStyle]::Italic)
    $grpObsCtl.Controls.Add($lblCutFKeyTip)

    $y += 270 + 10
    $y += 40  # Additional offset for expanded OBS Control group
...
function Start-AutoMode {
    try {
        ...
        # Channels 2-9: Media channel stays at 0 dB, others use Auto Mode idle baseline
        $mediaChStart = [int]$script:Cfg.XR.MediaChannel
        $idleLowDB = -30.0
        $idleHighDB = -23.0
        $idleMidDB = ($idleLowDB + $idleHighDB) / 2.0
        $linIdle = ConvertTo-LinearFader $idleMidDB
        for ($am = 2; $am -le 9; $am++) {
            if ($am -eq $mediaChStart) {
                $linMedia = ConvertTo-LinearFader 0.0
                XR-WriteFaderPosition $am $linMedia
                Update-AutoModeFaderDisplay $am 0.0
                $script:_autoModeLastDB[$am] = 0.0
            }
            else {
                XR-WriteFaderPosition $am $linIdle
                Update-AutoModeFaderDisplay $am $idleMidDB
                $script:_autoModeLastDB[$am] = $idleMidDB
            }
        }
        ...
        $script:_autoModeTimer.Add_Tick({
                try {
                    ...
                    # Guard: ensure config exists before accessing
                    if (-not $script:Cfg -or -not $script:Cfg.XR) { return }
                    ...
                    $lowDB = -30.0
                    $waveMaxDB = -23.0
                    $waveMidDB = ($lowDB + $waveMaxDB) / 2.0
                    $waveAmp = ($waveMaxDB - $lowDB) / 2.0
                    $now = Get-Date
                    ...
                    # Guard: ensure hashtables exist
                    if (-not $script:_autoModeLastDB) { $script:_autoModeLastDB = @{} }
                    if (-not $script:_autoModeChHoldUntil) { $script:_autoModeChHoldUntil = @{} }

                    # ---- Media channel: always pinned at 0 dB while Auto Mode is active ----
                    if ($mediaCh -ge 1 -and $mediaCh -le 30) {
                        $mediaLast = if ($script:_autoModeLastDB.Contains($mediaCh)) { [double]$script:_autoModeLastDB[$mediaCh] } else { -90.0 }
                        if ([Math]::Abs($mediaLast - $highDB) -gt 0.1) {
                            $linHigh = ConvertTo-LinearFader $highDB
                            try { XR-WriteFaderPosition $mediaCh $linHigh } catch {}
                            try { Update-AutoModeFaderDisplay $mediaCh $highDB } catch {}
                    ...
                    for ($am_ch = 2; $am_ch -le 9; $am_ch++) {
                        ...
                        $holdUntil = if ($script:_autoModeChHoldUntil.Contains($am_ch)) { $script:_autoModeChHoldUntil[$am_ch] } else { $null }
        ...
        Log "Auto Mode: Active — Ch1=0dB, Ch2-9 wave -30..-23dB (Media pinned at 0dB) with auto-raise on input"
    }
...
$script:form.Add_FormClosing({
        ...
        # Clean up Cut hotkey registration
        try { Unregister-CutHotKey } catch {}
