...
function Import-ConfigFromPath($configPath) {
    try {
        ...
        foreach ($section in @('UI', 'OBS', 'OBSControl', 'Music', 'Meeting', 'Zoom', 'Reminders', 'XR', 'Audio')) {
...
$script:meetingTimer.Add_Tick({
        try {
            ...
            if ($script:Cfg.OBSControl.AutoStartAutoToggle -and $sec -ge 0 -and
                ...
                # Reset guard so the silent path runs fresh for this auto-start
                $script:_zoomWarningShownThisToggle = $false
                Start-AutoToggle
                # Silently correct mic/camera without blocking dialog
            ...
            try {
                ...
                if ($script:Cfg.XR.LimiterEnabled) {
                    ...
                    for ($li = 0; $li -lt 9; $li++) {
                        ...
                        if ($limFaderDB -lt -0.5 -or $limFaderDB -gt 10.0) { continue }
